...
For Prim's algorithm we simply need a Frontier list that allows random access. This is just a buffer that stores the row and column for a cell.
...
In order for the BFS algorithm to work we need to track a visited list (easy since it is a grid, see. Visited buffer) and a queue with the cells we're currently exploring.
